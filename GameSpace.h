...
const int MaxGameSize=10 ; //taille max de l'espace jeu
...
class GameSpace {
    ...
    int dim ; // dimension de l'espace jeu dans lequel on va jouer
public:
    //****************declaration des methodes du classe*******
    GameSpace(int val=0) ;//constructeur avec le paramtre "val" initialise la dimension (0 par defaut)
    void DisplayCase(int) ; // affiche la case sous forme d'un symbole
    void DisplayGameSpace() ;// affiche l'espace jeu
    int GetCase(int x,int y) ; //retourne la case de coordonnèes (x,y)
