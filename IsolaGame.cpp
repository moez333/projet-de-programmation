...
Position IsolaGame::FindCaseToDestroyIA()
{
    ...
    while(test)
    {
        ...
        int nbrofplayer=(NbPlayers==2 ? 1 : rand()%(NbPlayers-NbIAmachines)+1) ; // s'il ya 2 joueurs : 1 joueur normal et une machine le IAmachine va se concentrer sur ce joueur si on a plusieurs joueurs normaux on choisit aléatoirement parmis ces joueurs
...
GameSpace IsolaGame::FutureGameSpace(GameSpace oldGameSpace , int wayToMove,Position activeplayerpos) {
    ...
    Position posold = activeplayerpos ; //pour conserver l'ancienne position
    SwitchCase(oldGameSpace,posnew,posold) ;  // nouvel etat de jeu
...
float IsolaGame::GetHeuristicOfGameState(Position activeplayerpos,Position opponentplayerpos ,GameSpace G) {
    float score = 0 ;
    //le nombre des coups légitimes pour les joueurs
   int player_moves,opponent_moves ;
    ...
    //s'il reste 60% espace vide dans le GameSpace , le bot devient plus agressif c.a.d l'heuristicscore devient 3 fois plus agressif que le joueur normal
    if(NbrGetEmptyCases() >= (ceil((G.getsize()*G.getsize()-2)*0.6)))
    {
        score=player_moves-(3*opponent_moves) ;
    }
    //s'il reste plus de 30% espace vide dans le  GameSpace , le bot devient plus agressif c.a.d l'heuristicscore devient 2 fois plus agressif que le joueur normal
...
float IsolaGame::maxval( GameSpace G ,Position activeplayerpos,Position oppenentplayerpos , int depth , float alpha , float beta) {
    ...
    float min_score ;
    if(depth == 0 || activeplayerpos.ListOfPlayerLegalMoves(G)==0) //si la profondeur de l'arbre = 0 ou "given player" n'admet pas d'accés à des mouvements on va retourner le score
    {
